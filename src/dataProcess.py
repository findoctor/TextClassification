...
category = {
    unicode('earn'): [152, 40, 0],
    unicode('acq'): [114, 25, 1],
    unicode('crude'): [76, 15, 2],
    unicode('corn'): [38, 10, 3]
}
nltk.download('punkt')
...
def trim_docs(text, blacklist):
    ...
    filtered = [i for i in word_tokenize(text.lower()) if i not in stop]
...
def save_into_pickle(train_data, test_data):
    with open('train_data_small.pkl', 'wb') as handle:
        pickle.dump(train_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open('test_data_small.pkl', 'wb') as handle2:
...
'''
train_path = 'train_data_small.pkl'
test_path = 'test_data_small.pkl'
train_small_data, test_small_data = load_pickled_data(train_path, test_path)
print(train_small_data[13])
